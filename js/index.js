// GAME CONSTANTS
const BOARD_SIZE = (window.innerWidth / (window.innerWidth / 0.95)) * 100; // Board size is based on percentage of width of browser window
const CARD_COLUMNS = 6;
const TILE_SIZE = (BOARD_SIZE / CARD_COLUMNS) * 0.82;
...
const gameStates = {
  ...
  lost: {
    message: "Oh no!",
    submessage: `You managed to match ${
      matches / 2
    } Pokémon. You were missing ${MAX_MATCHES - matches / 2}.`,
  },
};
...
board.addEventListener("click", (event) => {
  ...
  if (
    event.target.classList.contains("card-back") ||
    event.target.classList.contains("card-front")
  ) {
